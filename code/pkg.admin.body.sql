...
create or replace package body admin as
	...
	Begin
		if a_cr_privilege_id is null then
			v_cr_privilege_id:=apptools.get_privilege_id(a_privilege_code);
		end if;
		...
		if a_cr_role_id is null then
			Begin 
				select cr_role_id into v_cr_role_id from cr_role where code=a_krn_role_code;
		...
		insert into cr_right (
			...
			krn_right_seq.nextval,
			a_table_name,	
			a_column_name,
			a_row_id,	
			a_code,			
			nvl(v_privilege_id,a_privilege_id),
	...
	Begin
		...
		r_cr_menu.privilege_id			:=apptools.get_privilege_id(v_privilege_code);		
		...
		Begin
			select krn_module_id into r_cr_menu.krn_module_id from krn_module where code=upper(v_module_code);
			exception when no_data_found then null;
		End;
		--dbms_output.put_line('r_cr_menu.krn_module_id='||to_char(r_cr_menu.krn_module_id));
		if r_cr_menu.cr_menu_id is null then
			r_cr_menu.cr_menu_id:=krn_menu_seq.nextval;
			insert into cr_menu values r_cr_menu;
		else 
			update cr_menu set row=r_cr_menu
			where cr_menu_id= r_cr_menu.cr_menu_id;
		end if;
		insert into krn_right (KRN_RIGHT_ID, code, privilege_id, krn_role_id)
		select krn_right_seq.nextval, code, apptools.get_list_value_id('PRIVILEGE','MAIN_MENU'), (select krn_role_id from krn_role where code='DBA') 
		from cr_menu km where  km.cr_menu_id=r_cr_menu.cr_menu_id
		and not exists (select 1 from krn_right r where r.code=km.code and r.privilege_id=apptools.get_list_value_id('PRIVILEGE','MAIN_MENU') and r.krn_role_id=(select krn_role_id from krn_role where code='DBA'));
